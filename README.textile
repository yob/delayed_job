...
By default, workers will loop forever or until killed. If Delayed::Worker.seconds is set to a number >= 0 then the
workers will exit after approximatly that number of seconds.

By default all jobs are scheduled with priority = 0, which is top priority. You can change this by setting
Delayed::Worker.default_priority to something else. Lower numbers have higher priority.
...
Delayed::Worker.seconds = 5.minutes
